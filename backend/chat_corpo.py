...
# Função principal que será chamada pelo backend
...
def calcular_alertas_jornada(dados_paciente):
    ...
    # Se não há tratamento iniciado e já se passaram mais de 7 dias do diagnóstico, irá ter um alerta
...
# Adicione isso no final do seu script para testar
if __name__ == '__main__':
    import json # para formatar a saída

    # [cite_start]1. Simule os dados de um paciente (peguei o primeiro do CSV [cite: 100])
    dados_paciente_exemplo = {
        'patient_id': 'P-373C',
        'nome_paciente': 'Marcelo Melo',
        'idade': '26',
        'diagnostico_data': '2025-07-01',
        'tipo_cancer': 'Câncer de próstata',
        'estadiamento': 'IIA',
        'cirurgia_data': '',
        'quimioterapia_inicio': '2025-11-10',
        'radioterapia_inicio': '2025-09-20'
    }

    # 2. Simule uma mensagem do paciente
    mensagem_paciente_exemplo = "Olá doutor, não estou me sentindo bem essa semana. Tive muita dor de cabeça e um pouco de enjoo depois da sessão de radio. O remédio para náusea não adiantou muito. Devo me preocupar?"

    # 3. Chame sua função principal
    #print("Enviando dados para a LLM...")
    resultado_json = gerar_resumo_medico(dados_paciente_exemplo, mensagem_paciente_exemplo)

    # 4. Imprima o resultado
    if resultado_json:
        print("\n--- Resposta da LLM ---")
        # Converte a string JSON para um objeto Python e imprime de forma legível
        resposta_formatada = json.loads(resultado_json)
        print(json.dumps(resposta_formatada, indent=2, ensure_ascii=False))
    else:
